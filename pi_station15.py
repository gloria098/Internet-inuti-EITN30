...
import time, struct, board, os, select, fcntl, sys, subprocess, socket
import threading 
#import chardet, pyx
...
def radio_TX(buffer):
    ...
    result = nrf.send(buf = buffer, ask_no_ack=False, send_only=True) #ack is enabled
...
def radio_RX(tun_fd):
    ...
    while True:
        #print("nrf.available() |||||||||||| ", nrf.available())
